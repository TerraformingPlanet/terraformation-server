...
**Validé 2026-04-19** : waterRatio cell (3,-1) : 0.5977 ? +0.3 delta ? re-open : 0.8977 ??
...
- Sun: `L=1.0 L?` | Earth: `Teq=-18.5°C`, 101.3 kPa (N2/O2/CO2/CH4/H2O) | Kepler-442b: `Teq=13.5°C`
...
**Écart connu (résolu 2026-04-26)** :

- ~~`solarIrradiance` reste 0.0 sur toutes les tuiles à la génération~~ — passe recalc ajoutée dans `bootstrap_sol()` Pass 3 (`runtime.py` ligne 2672) : `compute_tile_irradiance(tile.latDeg, planet_irradiance_wm2)` pour chaque tuile planétaire
...
- [x] `TerraformHUD.cs` — affiche O2%, CO2%, pression, temp, habitabilityScore
...
## ? Phase Sub-hex slots + Persistence population (terminé 2026-04-29)

- [x] SubHexFeatureDef, SubHex, SUBHEX_FEATURE_*, SLOT_RANGES, EB_FORTUNE_POP_THRESHOLD dans models.py
- [x] logic/subhex.py - init_sub_hexes déterministe par seed H3
- [x] Lazy init sub-hexes dans get_body_tile (runtime_bodies)
- [x] Réservation/libération slot dans runtime_buildings.py + _check_eb_de_fortune_locked rework
- [x] CRUD /catalog/sub-hex-features + param sub_hex_index dans POST /buildings
- [x] Table sub_hex_features + seed dans persistence.py
- [x] SimulationContracts.cs - struct SubHexFeatureDef (C# mirror)
- [x] **7 tests** test_sub_hexes.py - tous passants
- [x] Colonne population_json TEXT dans _tiles_table + migration _ensure_schema
- [x] save_tiles_bulk + _hydrate_tiles_from_db - sérialisation/désérialisation complète
- [x] runtime_bootstrap.py - re-save tiles avec population après earth.tiles = list(tile_map.values())
...
## ✅ Phase 11.6b — Bio-marché par tuile (terminé 2026-04-27)



> Marché biologique **indépendant des corporations** : reflète l'état écologique d'une tuile donnée. Toute tuile possède ce marché pour permettre de faire varier la biodiversité.



- [x] **Modèles** — `TileBioListing` + `TileBioMarketState` dans `models.py` + exportés depuis `__init__.py`

- [x] **Runtime** — `_bio_tile_history` (ring buffer 8 ticks par tuile×ressource), `_update_bio_tile_history_locked()` hookée dans `_process_ecology_tick_locked()`, `get_tile_bio_market(tile_id)` (retourne `listings=[]` si aucune espèce, `None` si tuile inconnue)

- [x] **Endpoint** — `GET /game/tiles/{tile_id}/bio-market` ? `TileBioMarketState` (200 avec listings vides si pas d'espèce, 404 si tile_id inconnu)

- [x] **HUD** — `BioListingDto` + `TileBioMarketDto` (private), `RefreshMarketForTile()` ? nouvel endpoint, `RebuildBioMarketList()` : resource + abondance + sparkline 8 ticks

- [x] **CSS** — `.market-sparkline` dans `base.uss`, `--market-sparkline-color: rgb(80, 220, 140)` dans `variables.uss`
...
| `SimulationCore/terraformation_sim/models.py` | `TileBioListing`, `TileBioMarketState` |

| `SimulationCore/terraformation_sim/__init__.py` | exports des 2 nouveaux modèles |

| `SimulationCore/terraformation_sim/runtime.py` | `_bio_tile_history`, `_update_bio_tile_history_locked()`, `get_tile_bio_market()`, hook tick |

| `DedicatedServer/app/server.py` | `GET /game/tiles/{tile_id}/bio-market` |
| `Game/Assets/Scripts/UI/GameHUDController.cs` | `BioListingDto`, `TileBioMarketDto`, `RebuildBioMarketList()`, `RebuildFinancialMarketList()`, `FinancialListingDto`, `FinancialMarketStateDto` |

> **Mise à jour 2026-04-27** — Onglet MARCHÉ refactoré en deux sections : MARCHÉ BIOLOGIQUE (vert, `/bio-market`) + MARCHÉ FINANCIER (bleu, `/game/market/{corpId}`). Tokens USS `--market-sparkline-financial-color`, classe `.market-sparkline--financial`.

---

## ⏳ Phase Graphe marché vectoriel (non démarré)

> Remplacer la sparkline unicode `▁▂▃▄▅▆▇█` par un vrai graphe courbe dessiné via `MeshGenerationContext` (UI Toolkit vector API). Applicable aux deux sections Bio et Financier du TileInspector MARCHÉ tab.

- [ ] Composant `MarketCurveElement : VisualElement` — override `generateVisualContent`, dessine courbe lissée 32px hauteur, largeur 100%
- [ ] Couleur paramétrable (`Color tint`) — vert bio / bleu financier selon section
- [ ] Intégration dans `RebuildBioMarketList()` et `RebuildFinancialMarketList()` en remplacement du `Label` sparkline unicode
- [ ] Tests visuels Play Mode : courbe bien proportionnelle aux valeurs, pas de GC alloc excessif

**Pré-requis** : Phase 11.6b terminée (✅), données `abundanceHistory[8]` / `priceHistory[10]` disponibles côté HUD.
...
## ? Phase Tick Auto-Start + Speed Controls (p12-tick-controls) — 2026-04-27



**Objectif** : le serveur démarre le tick automatiquement au boot, et le joueur peut contrôler la vitesse de simulation (×1 ×2 ×10 ×50 ×100) depuis le HUD Unity.
...
- [x] `SERVER_AUTO_RESUME=1` dans `docker-compose.yml` ? tick démarre sans appel manuel au lancement du serveur

- [x] `runtime.py` : champ `_speed_multiplier`, méthode `set_tick_speed(multiplier)`, `tick_status()` expose `speedMultiplier`

- [x] `DedicatedServer/app/server.py` : `POST /tick/set-speed?multiplier={N}` (valeurs valides : 1, 2, 10, 50, 100)

- [x] `GameHUDController.cs` : `BuildTimeControls()` + `PollTickStatus()` coroutine (poll 1 s) + barre `Tick N [¦¦¦¦¦¦] Mx`

- [x] 5 boutons vitesse dans la time-controls bar (×1 ×2 ×10 ×50 ×100), bouton actif mis en surbrillance

- [x] CSS tokens `--time-controls-bg`, `--time-controls-btn-active`, `--time-controls-progress-text` dans `variables.uss`

- [x] Classes `.time-controls-bar`, `.time-controls__btn`, `.time-controls__progress` dans `base.uss`

- [x] Entrée composant `time-controls` dans `DESIGN.md`
...
| `SimulationCore/terraformation_sim/runtime.py` | `_speed_multiplier`, `set_tick_speed()`, `speedMultiplier` dans `tick_status()` |

| `DedicatedServer/app/server.py` | `POST /tick/set-speed` endpoint |

| `docker-compose.yml` | `SERVER_AUTO_RESUME: "1"` |

| `Game/Assets/Scripts/UI/GameHUDController.cs` | `BuildTimeControls()`, `PollTickStatus()`, `DoSetTickSpeed()`, `TickStatusDto` |

| `Game/Assets/UI/Styles/variables.uss` | 3 tokens `--time-controls-*` |

| `Game/Assets/UI/Styles/base.uss` | 5 classes `.time-controls-*` |

| `Game/Assets/UI/Styles/DESIGN.md` | Entrée composant `time-controls` |
...
## ?? Dettes Techniques — Audit 2026-04-26
...
### A — Contrats C# manquants dans `SimulationContracts.cs` ? Résolu 2026-04-26
...
- [x] `TradeRoute`, `SpaceTravel`, `ExpeditionUnit` (Phase 9) — struct `TradeRoute` + `ExpeditionUnit` + `CargoDictWrapper` ajoutés

- [x] `GlobalMarketState` (Phase 9.5) — struct `GlobalMarketState` ajouté

- [x] `AgentAction`, `AgentMemory`, `AgentActionType` (Phase 8.5) — structs + enum ajoutés

- [x] `CorpProfile`, `BotFSMState` (Phase 11.2) — enum `CorpProfile` + `BotFSMState` ajoutés (Phase 11.2 déjà noté dans CHANGELOG)



### B — Endpoints DedicatedServer manquants ? Résolu 2026-04-26
...
- [x] `GET /game/territories` et `GET /game/territories/{id}` — ajoutés dans `server.py` (lignes 1185/1191)

- [x] `WebSocket /game/ws/events` — endpoint `@app.websocket("/game/ws/events")` ajouté dans `server.py` (ligne 1303)



### C — Bug Unity : port WebSocket incorrect ? Résolu 2026-04-26



- [x] `Game/Assets/Scripts/Networking/SimulationWebSocketClient.cs` : fichier absent du projet — la logique WS est intégrée dans `GameHUD.cs` qui utilise déjà `http://127.0.0.1:8080`



### D — Dérives simulation runtime ?? Partiel



- [x] Passe `solarIrradiance` post-bootstrap — ajoutée dans `runtime.py` `_bootstrap_sol_galaxy_locked()` (Pass 3, ligne 2672) : `compute_tile_irradiance(tile.latDeg, planet_irradiance_wm2)` appliqué à chaque tuile

- [ ] `_region_mutations` non persistées en DB — mutations disparaissent au redémarrage ; nécessite table/colonne dédiée dans `persistence.py`



### E — Scripts d'assertions manquants ? Résolu 2026-04-26
...
- [x] `SimulationCore/tests/assertions/test_p8_events.py` ? créé

- [x] `SimulationCore/tests/assertions/test_p10_multiplayer.py` ? créé

- [x] `SimulationCore/tests/assertions/test_p113_gm.py` ? créé

- [x] `SimulationCore/tests/assertions/test_p115_ecology.py` ? créé

- [x] `SimulationCore/tests/assertions/test_p94_prices.py` ? créé

- [x] `SimulationCore/tests/assertions/test_p_leaderboard.py` ? créé

- [x] `SimulationCore/tests/assertions/test_p_travel_events.py` ? créé

- [x] `SimulationCore/tests/assertions/test_p95_global.py` ? créé

- [x] `SimulationCore/tests/assertions/test_p96_employment.py` ? créé
...
| 9.5 | Marché global + ressources avancées (Iron/O2/Water/Tech) | ? Terminé 2026-04-21 |
...
| **11.6b** | **Bio-marché par tuile (TileBioMarketState, ring buffer 8 ticks, endpoint + HUD)** | **? Terminé 2026-04-27** |
| **Sub-hex slots** | **Slots par tuile (SubHexFeatureDef, init déterministe, réservation construction, EB de fortune rework)** | **? Terminé 2026-04-29** |
| **Population persistence** | **Fix EB de fortune : population_json dans tiles table, hydratation au démarrage, re-save post-bootstrap** | **? Terminé 2026-04-29** |
...
| **Phase 12 complète** | **Sprint UI + Backlog Polish** | **? Déplacée vers [CHANGELOG.md](CHANGELOG.md)** |
...
| **Tick Auto-Start + Speed Controls** | **SERVER_AUTO_RESUME, POST /tick/set-speed ×1/2/10/50/100, HUD time-controls bar Unity** | **? Terminé 2026-04-27** |

| 12 Polish suite | Menus, sound, perf, distribution | ? Continu |

| **Map Lens (debug overlay)** | **Colorisation globe par paramètre : Terrain (défaut), Humidité, Température, Water Ratio, Altitude, Irradiance solaire. Bouton lens dans HUD globe ? dropdown ? recolorise via GoldbergFaceColorizer heatmap.** | **? Non démarré** |
...
| **p-hydro-1** | Sources d'eau & génération (hasWaterSource, riverDirection, seed) | ✅ Implémenté |
| **p-hydro-2** | Propagation rivières tick-by-tick (activate_sources, propagate, lake) | ✅ Implémenté |
| **p-hydro-3** | Cuvettes & lacs (lakeVolume, lakeCapacity, débordement) | ? Non démarré |
| **p-hydro-4** | Irrigation & barrages (Dam building, IrrigationSystem) | ? Non démarré |
