...
description: "Use when marking a roadmap task or phase as complete: checking a [ ] box in ROADMAP.md, validating exit criteria, or moving a completed phase to CHANGELOG.md. Trigger words: marquer comme terminé, cocher, [ ] → [x], phase complete, compléter la phase, roadmap, Set-PhaseComplete, roadmap_complete_phase."
...
- **roadmap.json** monté en lecture seule dans le container (seed uniquement — les statuts sont gérés exclusivement par la DB Docker)
...
| `Documentation/roadmap.json` | Seed canonique — **jamais modifier directement pour les statuts** ; source de vérité = DB Docker |
...
## ⚠️ Sync DB Docker — OBLIGATOIRE après ajout d'une nouvelle phase via `roadmap_add_phase`

Après chaque `git pull` ou seed initial, synchroniser la DB Docker depuis `Documentation/roadmap.json` :
...
- Fichiers impliqués :
  ...
  - Roadmap Service : `http://localhost:8001`
...
4. Via Roadmap Service : `POST http://localhost:8001/phases/<id>/complete` (ou `roadmap_complete_phase("<id>", force=True)`)
...
- `Invoke-RestMethod http://localhost:8001/phases/<id>` → `status == "done"`
```
